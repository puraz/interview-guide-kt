package com.example.framework.base.result
...
class ApiResult<T> {
    ...
    /** 是否成功 */
    @get:Schema(description = "是否成功")
    val success: Boolean
        get() = code == ErrorCodeEnum.SUCCESS.code

    /** 状态信息 */
    @field:Schema(description = "状态信息")
    var message: String? = null

    /** 返回数据 */
    @field:Schema(description = "返回数据")
    var data: T? = null
    ...
    @field:Schema(description = "数据类型")
    var dataType: Int = DataTypeEnum.NORMAL.code

    /** 是否执行默认操作 */
    @JsonIgnore
    @field:Schema(description = "是否执行默认操作", hidden = true)
    var defExec = true
    ...
    constructor(
        code: Int? = ErrorCodeEnum.SUCCESS.code, message: String? = null, data: T? = null, error: String? =
            null, dataType: Int? = DataTypeEnum.NORMAL.code
    ) {
        this.code = code
