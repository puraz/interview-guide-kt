package com.example.framework.core.satoken.aspect

import cn.dev33.satoken.exception.NotPermissionException
import cn.dev33.satoken.stp.StpUtil
import cn.dev33.satoken.util.SaFoxUtil
import cn.hutool.core.util.StrUtil
import com.example.framework.core.satoken.annotation.*
...
class PreCheckAspect(private val ignoreProperties: IgnoreProperties) {
    companion object {
        ...
        const val POINTCUT_SIGN: String =
            "@within(com.example.framework.core.satoken.annotation.PreCheckPermission) || " +
                    "@annotation(com.example.framework.core.satoken.annotation.PreCheckPermission) || " +
                    "@within(com.example.framework.core.satoken.annotation.PreCheckRole) || " +
                    "@annotation(com.example.framework.core.satoken.annotation.PreCheckRole)"
    }
    ...
    fun doAround(joinPoint: ProceedingJoinPoint): Any? {
        ...
        // 检查是否有免登录注解
        if (hasSkipLoginAnnotation(method)) {
            enabled = false
        }
     ...
     * @return Boolean
     */
    private fun hasSkipLoginAnnotation(method: Method): Boolean {
        // 检查方法上的注解
        if (method.isAnnotationPresent(SkipLogin::class.java)) {
            return true
        }
        // 检查类上的注解
        if (method.declaringClass.isAnnotationPresent(SkipLogin::class.java)) {
            return true
        }
        return false
    }
