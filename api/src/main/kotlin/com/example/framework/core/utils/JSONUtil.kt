...
object JSONUtil {
     ...
     *
     * 说明：
     * 该方法是parseObject的inline简化版，使用时无需传入TypeReference参数
     *
     * 使用方法：
     * ```
     * // 将jsonStr转成List<SysUser>对象
     * val userList: List<SysUser> = json.parseList<SysUser>()
     * ```
     * @param T 泛型
     ...
     * json字符串转对象
     * 用法：
     * val list: List<String> = output.parseAs<List<String>>()
     * val map: Map<String, Any> = output.parseAs<Map<String, Any>>()
     */
    inline fun <reified T> String.parseAs(): T {
        return parseObject(
            this,
            object : TypeReference<T>() {}
        ) ?: throw BusinessException("结果解析失败")
    }
