package com.example.framework.core.log.aspect

import cn.hutool.core.exceptions.ExceptionUtil
import cn.hutool.core.util.StrUtil
import cn.hutool.http.useragent.UserAgentUtil
import com.example.framework.core.log.annotation.SysLog
import com.example.framework.core.log.enums.LogTypeEnum
import com.example.framework.core.log.event.LogEvent
import com.example.framework.core.log.model.LogDTO
import com.example.framework.core.utils.IpAddressUtil
import com.example.framework.core.utils.JSONUtil
import com.example.framework.core.utils.ServletUtil
...
class LogAspect(private val context: ApplicationContext) {
    ...
    @Pointcut("@annotation(com.example.framework.core.log.annotation.SysLog)")
    fun LogAspect() {
    }
    ...
    private fun getDescription(joinPoint: JoinPoint, sysLog: SysLog): String {
        ...
        // 获取 @Tag 的 name 值（用于日志展示的“模块前缀”）
        val tag = joinPoint.target.javaClass.getAnnotation(Tag::class.java)
        ...
        if (StrUtil.isNotBlank(sysLog.value)) {
            ...
            // 获取 @Operation 的 summary 值
            val signature = joinPoint.signature as MethodSignature
            val method = signature.method
            if (method.isAnnotationPresent(Operation::class.java)) {
                val operation = method.getAnnotation(Operation::class.java)
                if (StrUtil.isNotBlank(operation.summary)) {
                    sb.append(operation.summary)
                } else {
                    // @SysLog 没有 value 值、@Operation 没有 summary 值的情况下：显示方法名
                    sb.append(method.name)
                }
            } else {
                // @SysLog 没有 value 值、没有 @Operation 注解的情况下：显示方法名
                sb.append(method.name)
            }
        }
        return sb.toString()
